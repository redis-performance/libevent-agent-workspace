...
| Guard `EVBASE_NEED_NOTIFY(base)` in `event_callback_activate_nolock_` behind `base->th_base_lock != NULL` (Tier 6b) | **Below noise floor.** cascade_bench: 107→108 µs (+0.9%, noise), cascade_chain: 154→154 µs (0%). `EVBASE_NEED_NOTIFY` already short-circuits on `evthread_id_fn_ == NULL` (1 BSS load, ~4 ns). Adding `th_base_lock != NULL` guard merely swaps one pointer load for another; net saving ≈ 0 ns. For 100 activations per run: ~400 ns expected, permanently below the 5–7 µs noise floor. This closes the last formally untried Tier 6 technique. **All program.md Tier 3–6 techniques are now exhausted for the cascade benchmark suite.** Future progress requires an evbuffer-based benchmark (bench_http + wrk) to expose Tier 1–2 techniques. |
